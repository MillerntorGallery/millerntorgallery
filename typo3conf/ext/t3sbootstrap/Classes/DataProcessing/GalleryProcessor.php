...
class GalleryProcessor implements DataProcessorInterface
{
	...
	public function process(
		...
		$imageOnly = FALSE;
		if (empty($processedData['data']['bodytext']) && empty($processedData['data']['header']) && count($this->fileObjects)) {
			$imageOnly = TRUE;	
		}

		$this->determineGalleryPosition();
		$this->determineMaximumGalleryWidth($imageOnly);

		$this->calculateRowsAndColumns();
		if (count($this->fileObjects))
		$this->calculateMediaWidthsAndHeights();
		if (count($this->fileObjects))
		$this->prepareGalleryData();

		$gridNumbers = (int)$this->getConfigurationValue('gridNumbers') ?: 12;

		$this->galleryData['col-md'] = (int)$gridNumbers/$this->numberOfColumns;

		$maxNumber = $this->numberOfColumns * $this->galleryData['count']['rows'];

		if (count($this->fileObjects)) {
			$this->galleryData['chunk'] = array_chunk($this->fileObjects, $this->numberOfColumns);
			$this->galleryData['mod'] = FALSE;
			if($maxNumber %count($this->fileObjects) != 0 || count($this->fileObjects) < $maxNumber) {
				$this->galleryData['mod'] = TRUE;
				if ($this->numberOfColumns - ($maxNumber %count($this->fileObjects))) {
					$this->galleryData['col-md-last'] = (int)floor($gridNumbers / ($this->numberOfColumns - ($maxNumber %count($this->fileObjects))));					
				} else {
					$this->galleryData['col-md-last'] = $gridNumbers;						
				}
				if (count($this->fileObjects) == 1)
				$this->galleryData['col-md-last'] = $gridNumbers;
				#if (count($this->fileObjects) == 0)
				#$this->galleryData['col-md-last'] = $this->galleryData['col-md'];
			}
		}

		$this->galleryData['mediaClassCenter'] = '';
		$this->galleryData['mediaClassPosition'] = '';	

		if ( $this->galleryData['position']['horizontal'] == 'center' ) 
		{
			$this->galleryData['mediaClassCenter'] = 'center-block';	
		} 
		else 
		{
			$this->galleryData['mediaClassPosition'] = 'pull-'.$this->galleryData['position']['horizontal'];			
		}
	 ...
	 * @return void
	 */
	protected function determineMaximumGalleryWidth($imageOnly)
	{
